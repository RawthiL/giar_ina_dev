...
def metadata_balanced(metadata_df):
    ...
    mask = (metadata_df['Fondo'] == True) & metadata_df.loc[:, 'Interfase':'Desconocido'].eq(False).all(axis=1)
    metadata_df=metadata_df.drop(index=metadata_df[mask].index) #discart images that only contain background
    ...
    for col in metadata_df.columns[skip_cols:len(dict_attributes)+1]:  
        dict_len_categories[col] = metadata_df[col].sum() #Total observations per cell type
    ...
    if check_imbalance == True:
        ...
        for col in metadata_df.columns[skip_cols:len(dict_attributes)+1]:
            perc= metadata_df[col].sum()/total_dict  #Calculate the updated percentage after discarding rows for each feature in each cycle of the for loop
            if col !=min_cat and perc > per_max: # Exclude the feature with the lowest percentage and check the maximum percentage
                ...
                cat.remove(col)
                mask = (metadata_df[col]== True) & (metadata_df[min_cat]== False) & (metadata_df[col] == True) & (metadata_df.loc[:,cat].eq(False).all(axis=1))
                ...
                if len(df_tmp)>=nro_total_drop:
                    ...
                    metadata_df=metadata_df.drop(indices_to_drop)
                    df_tmp2=metadata_df[(metadata_df[col]== True) & (metadata_df[min_cat]== False)]
                ...
                total_dict = sum(dict_len_categories.values())
    print(f"updated total by category  :" )
    for key in dict_len_categories.keys():
        print(f"\t{key} = {dict_len_categories[key]}")
...
def create_dataset(dataset_path, dataset_name, metadata_df,  output_path=None):
    ...
    metadata_df['prefix'] = metadata_df['image'].apply(lambda x: '_'.join(x.split('_')[:2]))
    ...
    for i, v in train_df.iterrows():
        ...
        image_input_train = np.asarray(Image.open(os.path.join(dataset_path, "input", v.iloc[0])).convert('L'))
        if output_path is not None:
        ...
        image_target_train = np.asarray(Image.open(os.path.join(dataset_path, "target", v.iloc[0])).convert('L'))
        if output_path is not None:
        ...
        if count == 0:
            train_images = np.zeros((len(train_df), image_input_train.shape[0], image_input_train.shape[1], 1), dtype=np.uint8)
    ...
    for i, v in val_df.iterrows():
        # Leer y guardar la imagen de entrada
        image_input_val = np.asarray(Image.open(os.path.join(dataset_path, "input", v.iloc[0])).convert('L'))
        if output_path is not None:
            path_input_val = os.path.join(input_val_dir, v.iloc[0])
            cv.imwrite(path_input_val, image_input_val)

        # Leer y guardar la imagen de destino
        image_target_val = np.asarray(Image.open(os.path.join(dataset_path, "target", v.iloc[0])).convert('L'))
        if output_path is not None:
        ...
        if count == 0:
            validation_images = np.zeros((len(val_df), image_input_val.shape[0], image_input_val.shape[1], 1), dtype=np.uint8)
    ...
    metadata_new = pd.concat([metadata_new, pd.DataFrame(rows_train), pd.DataFrame(rows_val)], ignore_index=True)
    if output_path is not None:
        metadata_new.to_csv(f"{new_path}/metadata.csv", index=False)

    return train_images, train_target, validation_images, validation_target, metadata_new
