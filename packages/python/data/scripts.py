...
def cut_images(paths, height, width, overlapX, overlapY, metadata):
    ...
    for subfolder in paths:
        ...
        )  # I create all folderes if they do not exist
        row_index = 0
    ...
    for idx, image_file in enumerate(paths[subfolder]):
        ...
        # Cut image
        cuts_list = data_augmentation.cut_image(
            image, height, width, overlapX, overlapY
        )
        ...
        for idx, img_cut in enumerate(cuts_list):
            ...
            subimage = cv.cvtColor(img_cut, cv.COLOR_BGR2GRAY)
    ...
    if subfolder == metadata_dir and metadata:
        ...
        df = pd.DataFrame(
            subimages_classes, columns=data_utils.get_image_metadata_columns()
        )
